...
function FooterAccordion({ title, links }: { title: string; links: { label: string; href: string }[] }) {
  ...
  return (
    <div className="border-b border-white/10 lg:border-none">
      ...
      >
        <span className="text-xs font-semibold uppercase tracking-[0.15em] text-white/90">{title}</span>
        <ChevronDown className={cn('h-4 w-4 text-white/40 transition-transform', open && 'rotate-180')} />
      </button>
      <h3 className="mb-4 hidden text-xs font-semibold uppercase tracking-[0.15em] text-white/90 lg:block">
        {title}
      </h3>
      <ul className={cn(
        ...
        {links.map((link) => (
          <li key={link.href}>
            <Link
              href={link.href}
              className="text-[13px] text-white/55 transition-colors hover:text-gold-light"
            >
              {link.label}
            </Link>
          </li>
  ...
  return (
    <footer className="pb-24 md:pb-0">
      ...
      <div className="bg-green">
        <Container>
          <div className="grid gap-8 py-12 lg:grid-cols-5 lg:gap-10 lg:py-16">
            {/* Brand Column */}
              ...
              {hasSocial && (
                <div className="mt-6 flex gap-3">
                  {siteConfig.socialLinks.instagram && (
                    <a href={siteConfig.socialLinks.instagram} target="_blank" rel="noopener noreferrer" className="flex h-9 w-9 items-center justify-center rounded-full border border-white/15 text-white/50 transition-all hover:border-gold/40 hover:text-gold-light" aria-label="Instagram">
                  ...
                  {siteConfig.socialLinks.youtube && (
                    <a href={siteConfig.socialLinks.youtube} target="_blank" rel="noopener noreferrer" className="flex h-9 w-9 items-center justify-center rounded-full border border-white/15 text-white/50 transition-all hover:border-gold/40 hover:text-gold-light" aria-label="YouTube">
        ...
        <Container>
          <div className="flex flex-col items-center justify-between gap-2 py-4 sm:flex-row">
            ...
            <div className="flex items-center gap-1">
              <span className="text-[10px] text-white/25">Sacred protection &middot; Guided with purpose</span>
            </div>
          </div>
        </Container>
      </div>
