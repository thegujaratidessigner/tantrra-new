...
function LotusOrnament({ className }: { className?: string }) {
  return (
    <svg width="26" height="22" viewBox="0 0 32 28" className={className} fill="none" aria-hidden="true">
...
function CrescentMoon({ className }: { className?: string }) {
  return (
    <svg width="16" height="16" viewBox="0 0 22 22" className={className} fill="none" aria-hidden="true">
      <path d="M16 4C12 5 9 9 9 13C9 17 12 20 16 21C11 22 6 18 6 12C6 6 11 2 16 4Z" fill="currentColor" opacity="0.5" />
...
function BotanicalLeaves({ className, side }: { className?: string; side: 'left' | 'right' }) {
  return (
    <svg width="70" height="100" viewBox="0 0 120 180" className={className} fill="none" aria-hidden="true"
...
function ScenicBackground() {
  return (
    <div className="pointer-events-none relative h-[85px] w-full overflow-hidden opacity-35" aria-hidden="true">
      <svg viewBox="0 0 400 200" className="h-full w-full" preserveAspectRatio="xMidYMax slice">
        <defs>
          <linearGradient id="drawerSky" x1="0" y1="0" x2="0" y2="1">
          ...
          <radialGradient id="drawerSun" cx="50%" cy="35%" r="30%">
            <stop offset="0%" stopColor="#F0E6B8" stopOpacity="0.6" />
            <stop offset="100%" stopColor="#F0E6B8" stopOpacity="0" />
          </radialGradient>
        </defs>
        <rect width="400" height="200" fill="url(#drawerSky)" />
        <circle cx="200" cy="70" r="60" fill="url(#drawerSun)" />
...
function DrawerIntro() {
  return (
    <div className="relative overflow-hidden px-5 pt-0 pb-2">
      ...
      <div className="pointer-events-none absolute -top-1 right-0 w-[60px] h-[90px] opacity-20">
        <BotanicalLeaves side="right" className="w-full h-full" />
      </div>
      <div className="pointer-events-none absolute top-2 right-2 w-[70px] h-[40px] rounded-full bg-[#F0E6B8]/12 blur-2xl" />

      <div className="relative z-10">
        <p className="text-right text-[10px] italic text-[#7A9B68]/55 leading-snug tracking-wide font-[family-name:var(--font-cormorant)]">
          A calmer<br />brighter you<br />awaits
        </p>
        <h2 className="mt-0.5 text-[24px] font-semibold text-[#1B3D2F] leading-[1.1] tracking-[-0.01em] font-[family-name:var(--font-cormorant)]">
          Namaste
        </h2>
        <p className="mt-0.5 text-[12px] leading-[1.5] text-[#68645C]">
...
function DrawerSearch({ onClose }: { onClose: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="px-5 mb-3.5">
      <div className="relative flex items-center h-[48px] rounded-full bg-[#F0EBDF] border border-[#E2DCD0]/60">
        <Search className="absolute left-3.5 h-[15px] w-[15px] text-[#918A80]" strokeWidth={1.8} />
        <input
          type="text"
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder="Search rituals, products..."
          className="w-full h-full bg-transparent pl-10 pr-4 text-[13px] text-[#211D18] placeholder:text-[#918A80] focus:outline-none"
        />
      </div>
    </form>
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 items-center justify-between px-3.5 py-2">
        ...
        <span className="flex h-[40px] w-[40px] flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#B88A3B] to-[#C49A50] shadow-sm transition-transform duration-200 group-hover:translate-x-0.5 ml-2">
...
function SecondaryNavLink({ item, onClose }: { item: typeof secondaryLinks[number]; onClose: () => void }) {
  ...
  return (
    <Link
      href={item.href}
      onClick={onClose}
      className="flex items-center justify-between px-2 transition-colors hover:bg-[#F0EBDF]/60 rounded-lg"
      style={{ minHeight: 54 }}
    >
      <div className="flex items-center gap-2.5">
        <span className="flex h-[28px] w-[28px] items-center justify-center rounded-full bg-[#F0EBDF]">
          <Icon className="h-[14px] w-[14px] text-[#4A6B3C]" strokeWidth={1.8} />
        </span>
        <span className="text-[14px] font-medium text-[#26372C]">{item.label}</span>
      </div>
      <ChevronRight className="h-3.5 w-3.5 text-[#918A80]" strokeWidth={1.8} />
    </Link>
  );
}
...
function ConsultationCTA({ onClose }: { onClose: () => void }) {
  return (
    <Link
      href="/consultations"
      onClick={onClose}
      className="group flex items-center gap-3 rounded-[14px] bg-[#E8EFDC]/60 border border-[#C5D4B0]/30 px-4 py-3 transition-all duration-200 hover:bg-[#E0E9D3]/70 active:scale-[0.98]"
      style={{ minHeight: 72 }}
    >
      <span className="flex h-[38px] w-[38px] flex-shrink-0 items-center justify-center rounded-[10px] bg-[#4A6B3C] shadow-sm">
        <CalendarCheck className="h-[17px] w-[17px] text-white" strokeWidth={1.8} />
      </span>
      <div className="flex-1 min-w-0">
        <p className="text-[14px] font-semibold text-[#1B3D2F]">Book Your Consultation</p>
        <p className="text-[11px] text-[#68645C]">Personalized spiritual guidance</p>
      </div>
      <span className="flex h-[32px] w-[32px] flex-shrink-0 items-center justify-center rounded-full bg-[#4A6B3C] shadow-sm transition-transform duration-200 group-hover:translate-x-0.5">
        <ArrowRight className="h-[14px] w-[14px] text-white" strokeWidth={2.2} />
      </span>
    </Link>
  );
}
  ...
  return (
    <div className="border-t border-[#E2DCD0]/40">
      <button
        ...
        className="flex w-full items-center justify-between px-2 transition-colors hover:bg-[#F0EBDF]/40 rounded-lg"
        style={{ minHeight: 54 }}
        aria-expanded={expanded}
      >
        <div className="flex items-center gap-2.5">
          <span className="flex h-[28px] w-[28px] items-center justify-center rounded-full bg-[#F0EBDF]">
            <User className="h-[14px] w-[14px] text-[#4A6B3C]" strokeWidth={1.8} />
          </span>
          <span className="text-[14px] font-medium text-[#26372C]">Your Account</span>
        </div>
        <ChevronDown
          className={cn('h-3.5 w-3.5 text-[#918A80] transition-transform duration-200', expanded && 'rotate-180')}
          strokeWidth={1.8}
        />
      </button>
      <AnimatePresence>
        {expanded && (
          ...
          >
            <div className="pb-1.5 pl-2 space-y-0.5">
              {accountLinks.map((link) => {
                ...
                return (
                  <Link
                    ...
                    href={link.href}
                    onClick={onClose}
                    className="flex items-center gap-2.5 rounded-lg px-3 py-2 text-[13px] text-[#68645C] transition-colors hover:bg-[#F0EBDF]/50 hover:text-[#26372C]"
                  >
                    <Icon className="h-[13px] w-[13px] flex-shrink-0" strokeWidth={1.6} />
...
function SacredFooter() {
  return (
    <div className="mt-2 px-5 pb-3">
      ...
      <ScenicBackground />
      <p className="mt-1.5 text-center text-[10px] tracking-[0.15em] text-[#4A6B3C]/60 font-[family-name:var(--font-cormorant)]">
...
function SubMenuPanel({
  title, links, onClose, onBack,
}: {
  title: string; links: SubLink[]; onClose: () => void; onBack: () => void;
}) {
  return (
    <motion.div
      initial={{ x: '100%' }}
    ...
    >
      <div className="flex items-center gap-3 border-b border-[#E2DCD0]/40 px-4 py-3">
        <button
          onClick={onBack}
          className="flex h-8 w-8 items-center justify-center rounded-full bg-[#F0EBDF] transition-colors hover:bg-[#E8E2D4]"
          aria-label="Back"
        >
          <ChevronRight className="h-3.5 w-3.5 rotate-180 text-[#4A6B3C]" strokeWidth={2} />
        </button>
        <h3 className="text-[15px] font-semibold text-[#1B3D2F] font-[family-name:var(--font-cormorant)]">{title}</h3>
      </div>
        ...
        {links.map((link) => {
          ...
          return (
            <Link
              ...
              href={link.href}
              onClick={onClose}
              className="flex items-center gap-3 rounded-lg px-3 py-2.5 transition-colors hover:bg-[#F0EBDF]/60"
            >
              {Icon && (
              ...
              <div>
                <p className="text-[13px] font-medium text-[#26372C]">{link.label}</p>
                {link.description && (
                  <p className="text-[10px] text-[#918A80] leading-snug mt-0.5">{link.description}</p>
                )}
              </div>
            </Link>
          );
      ...
      {isOpen && (
        ...
        >
          ...
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute inset-0 bg-[rgba(20,25,20,0.50)] backdrop-blur-[3px]"
            onClick={handleClose}
          />
              ...
              >
                ...
                <div className="flex items-center justify-between px-4 pt-3 pb-0.5">
                  <button
                    ref={closeButtonRef}
                    onClick={handleClose}
                    className="flex h-[36px] w-[36px] items-center justify-center rounded-full transition-colors hover:bg-[#F0EBDF]"
                    aria-label="Close menu"
                  >
                    <X className="h-[20px] w-[20px] text-[#26372C]" strokeWidth={1.8} />
                  </button>
